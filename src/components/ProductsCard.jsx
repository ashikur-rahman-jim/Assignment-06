...
const ProductsCard = ({ product, carts, setCarts }) => {
    ...
    return (
        <div className="bg-white rounded-xl shadow-md p-6 flex flex-col justify-between">
            ...
            {/* Button */}
            <div className='mt-5'>
                <button type="button" onClick={handelSubscription} className="w-full py-2 px-4 bg-linear-to-t from-[#4F39F6] to-[#9514FA] text-white font-semibold rounded-full  cursor-pointer hover:opacity-90 transition">{isSubscribed ? "Subscribed" : "Buy Now"}</button>
            </div>
        </div>
    );
};
